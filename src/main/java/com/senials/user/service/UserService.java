...
public class UserService {
    ...
    // 사용자가 만든 모임 목록 조회
    public Map<String, Object> getUserMadeParties(int userNumber) {
        List<PartyBoard> partyBoards = partyBoardRepository.findByUser_UserNumber(userNumber);

        // PartyBoardDTO 리스트 생성
        List<PartyBoardDTO> partyList = partyBoards.stream()
                .map(partyBoard -> {
                    PartyBoardDTO dto = new PartyBoardDTO();
                    dto.setPartyBoardNumber(partyBoard.getPartyBoardNumber());
                    dto.setUserNumber(partyBoard.getUser().getUserNumber());
                    dto.setHobbyNumber(partyBoard.getHobby().getHobbyNumber());
                    dto.setPartyBoardName(partyBoard.getPartyBoardName());
                    dto.setPartyBoardDetail(partyBoard.getPartyBoardDetail());
                    dto.setPartyBoardOpenDate(partyBoard.getPartyBoardOpenDate());
                    dto.setPartyBoardStatus(partyBoard.getPartyBoardStatus());
                    dto.setPartyBoardViewCnt(partyBoard.getPartyBoardViewCnt());
                    dto.setPartyBoardLikeCnt(partyBoard.getPartyBoardLikeCnt());
                    dto.setPartyBoardReportCnt(partyBoard.getPartyBoardReportCnt());
                    return dto;
                })
                .toList();
        ...
        Map<String, Object> resultMap = new HashMap<>();
        resultMap.put("userNumber", userNumber);
        resultMap.put("madeParties", partyList);
